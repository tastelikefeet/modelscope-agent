...
- 短视频类型：适配较广，科普类、经济类、日常类均可，尤其包含报表、公式、图片、原理性解释的短视频
- 语言：不限，字幕和语音语种跟随你的原始query和文档材料
- 读取外部材料：支持纯文本和图文，可以将图片利用在短视频中
...
- 多模态模型测试范围：Gemini-flash-2.5，其他模型效果未测试
...
```text
生成一个描述大模型技术的短视频，阅读/home/user/topic.txt获取详细内容
```
...
4. 运行持续约30~40min左右。视频生成在output/final_video.mp4。生成完成后你可以查看这个文件，把不满足要求的地方汇总起来，输入命令行input中，工作流会继续改进。如果达到了要求，输入quit或者exit程序会自动退出。
...
1. 根据用户需求生成基本台本
    * 输入：用户需求，可能读取用户指定的文件
    * 输出：台本文件script.txt，原始需求文件topic.txt，短视频名称文件title.txt，文件列表docs.txt
2. 使用多模态模型解析图片内容
    * 输入：文件列表docs.txt
    * 输出：图片信息列表image_info.txt
3. 根据台本切分分镜设计
    * 输入：topic.txt，script.txt，image_info.txt
    * 输出：segments.txt，描述旁白、背景图片生成要求、前景manim动画要求、前景图片要求的分镜列表
4. 生成分镜的音频讲解
    * 输入：segments.txt
...
7. 根据语音时长生成manim动画代码
    * 输入：segments.txt，audio_info.txt，image_info.txt
    * 输出：manim代码文件列表 manim_code/segment_N.py，N为segment序号从1开始
8. 修复manim代码
    * 输入：manim_code/segment_N.py N为segment序号从1开始，code_fix/code_fix_N.txt 人工反馈的错误文件
    * 输出：更新的manim_code/segment_N.py文件
    * 注：本step在首次执行时跳过，仅用于人工反馈
9. 渲染manim代码
...
11. 生成背景，为纯色带有短视频title和slogans的图片
    * 输入：title.txt
    * 输出：background.jpg
12. 拼合整体视频
    * 输入：前序所有的文件信息
...
- fonts: 字体列表
- mllm: 和llm配置项相同，仅在前面增加了`mllm_`前缀，用于配置多模态大模型
- code_fix_round: 代码修复轮数，用于渲染manim代码时对代码修复，超过轮数仍然未修复会报错，默认为5
- mllm_fix_round: 使用MLLM进行动画检测的轮数，默认为1
