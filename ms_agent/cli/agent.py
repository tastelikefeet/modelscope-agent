...
class AgentCMD(CLICommand):
    ...
    def define_args(parsers: argparse.ArgumentParser):
        ...
        agent_sub = agent_parser.add_subparsers(dest="agent_action", metavar="ACTION")
        ...
        creds.add_argument("--token", default=None, help="API token (default: `ms login` / MODELSCOPE_API_TOKEN).")
        creds.add_argument("--endpoint", default=None, help="API endpoint (default: MODELSCOPE_ENDPOINT).")

        _fw_help = f"Agent framework ({_FW_LIST})"

        # ---- upload ----
        p_upload = agent_sub.add_parser(
            "upload", parents=[creds],
        ...
        p_upload.add_argument("-f", "--framework", required=True, help=_fw_help)
        p_upload.add_argument("-n", "--name", default=None,
                              help="Local agent name; auto-selects if only one exists, errors if multiple")
        p_upload.add_argument("-r", "--repo", required=True,
                              help="Remote repo identifier, supports owner/name format (e.g. user/my-agent)")
        p_upload.add_argument("--local-dir", default=None,
        ...
        p_download = agent_sub.add_parser(
            "download", parents=[creds],
            help="Download agent files from remote repository",
            formatter_class=RawDescriptionHelpFormatter,
            description="Download remote agent files and write to local workspace.",
            epilog=f"supported frameworks: {_FW_LIST}",
        )
        p_download.add_argument("-f", "--framework", required=True, help=_fw_help)
        p_download.add_argument("-r", "--repo", required=True,
                                help="Remote repo identifier, supports owner/name format (e.g. user/my-agent)")
        p_download.add_argument("-n", "--name", default=None,
                                help='Local agent name to write as (default: "default")')
        p_download.add_argument("--local-dir", default=None,
                                help="Override local workspace root (default: framework standard path)")
        p_download.add_argument("--target-framework", default=None,
        ...
        p_watch.add_argument("-n", "--name", default=None,
                             help="Agent name to sync (default: ALL agents in the workspace)")
        p_watch.add_argument("-r", "--repo", required=True,
                             help="Remote repo identifier, supports owner/name format (e.g. user/my-agent)")
        p_watch.add_argument("--local-dir", default=None,
                             help="Override local workspace root (default: framework standard path)")
        p_watch.add_argument("--pull", action="store_true",
        ...
        p_list.add_argument("--page", dest="page_number", type=int, default=1,
                            help="Page number for pagination (default: 1)")
        p_list.add_argument("--page-size", dest="page_size", type=int, default=10,
        ...
        p_status.add_argument("-f", "--framework", required=True, help=_fw_help)
        p_status.add_argument("--local-dir", default=None,
                              help="Override local workspace root (default: framework standard path)")
        ...
        p_backups.add_argument("-f", "--framework", default=None,
                               help="Filter backups by framework name prefix")
        p_backups.add_argument("-n", "--name", default=None,
                               help="Filter backups by agent name (matches _{name}_ in filename)")
        p_backups.add_argument("--local-dir", default=None, help="Override local workspace root")
        ...
        p_restore.add_argument("--from-backup", required=True,
                               help="'last' (most recent matching backup) or a specific backup filename")
        p_restore.add_argument("-f", "--framework", default=None,
                               help="Filter backup candidates by framework (used with 'last')")
        p_restore.add_argument("-n", "--name", default=None,
                               help="Filter backup candidates by agent name (used with 'last')")
        p_restore.add_argument("--local-dir", default=None, help="Override restore target directory")
        ...
        p_convert.add_argument("--from-framework", required=True,
                               help=f"Source framework to read from ({_FW_LIST})")
        p_convert.add_argument("--target-framework", required=True,
                               help=f"Target framework to write to ({_FW_LIST})")
        p_convert.add_argument("--from-name", default=None,
                               help='Source agent name to read (default: "default")')
        p_convert.add_argument("--target-name", default=None,
                               help="Target agent name to write as (default: same as --from-name)")
        p_convert.add_argument("--local-dir", default=None,
                               help="Source workspace root to read from (default: source framework path)")
        p_convert.add_argument("--out-dir", default=None,
    ...
    def execute(self) -> None:
        from ms_agent.agent_hub import (
            cmd_backups,
            cmd_convert,
            cmd_download,
            cmd_list,
