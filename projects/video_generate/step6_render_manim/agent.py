        ...
        for i in range(5):
            ...
            try:
                result = subprocess.run(
                    ...
                    encoding='utf-8',
                    errors='ignore',
                    timeout=120,  # Reduced timeout to 120 seconds for faster failure detection
                    env=env)
                
                output_text = (result.stdout or '') + (result.stderr or '')
            except subprocess.TimeoutExpired as e:
                logger.error(f'Manim rendering timed out after 120 seconds for {actual_scene_name}')
                # Capture any partial output from the timeout
                output_text = (e.stdout.decode('utf-8', errors='ignore') if e.stdout else '') + \
                             (e.stderr.decode('utf-8', errors='ignore') if e.stderr else '')
                logger.error(f'Timeout output: {output_text[:500]}')
                # Treat timeout as an error and attempt to fix the code
                code = self.fix_manim_code(
                    f"Rendering timed out after 120 seconds. This usually indicates an infinite loop or hanging process.\n\nPartial output:\n{output_text}",
...
**Common Manim API Issues to Avoid**:
- Do NOT use `scale_tips` parameter - it doesn't exist in Manim's scale() method
- Use `scale()` without the scale_tips parameter
- Arrow scaling: Use `arrow.scale(factor)` directly, not `arrow.scale(factor, scale_tips=True)`
- For arrow tip scaling, recreate the arrow with different tip_length instead
...
- There may be some beginner's error because the code was generated by an AI model
    ...
    * Pay attention to invalid Manim API usage like scale_tips parameter
