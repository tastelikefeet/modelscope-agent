...
class Segment(LLMAgent):
    ...
    system = """You are an animation storyboard designer.
Now there is a short video scene that needs storyboard design.
The storyboard needs to meet the following conditions:
...
- Each storyboard panel will carry a piece of narration, one {animation_engine} animation(optional),
  one image background, and one subtitle, even one video(optional)
    ...
    * If no {animation_engine} needed, do not output the {animation_engine} key

- Each of your storyboard panels should take about 5~8 seconds to read at normal speaking speed.
  Avoid the feeling of frequent switching and static
        * Pay attention to the color and content coordination
          between the background image and the {animation_engine} animation.
    * If a {animation_engine} animation exists, the background image should not be too flashy.
      Else the background image will become the main focus, and the image details should be richer
    * The foreground and the background should not describe the same thing.
      For example, draw birds at the foreground, sky and clouds at the background,
      other examples like charts and scientist, cloth and girls
    * If a storyboard panel has {animation_engine} animation,
      the image should be more concise, with a stronger supporting role

{video_prompt}

- Write specific narration for each storyboard panel, technical animation requirements,
  and **detailed** background image requirements
    * Specify your expected {animation_engine} animation content, presentation details, position and size, etc.,
      and remind the large model generating {animation_engine} of technical requirements,
      and **absolutely prevent size overflow and animation position overlap**
    * Estimate the reading duration of this storyboard panel
      to estimate the duration of the {animation_engine} animation.
      The actual duration will be completely determined in the next step of voice generation
    * The video resolution is around 1920*1080, **the ratio of the {animation_engine} size is 16:9**.
    * Use thicker lines to emphasis elements
    * Use small/medium size of font/elements in {animation_engine} animations to prevent from cutting off by the edge
    * LLMs excel at animation complexity, not layout complexity.
        - Use multiple storyboard scenes rather than adding more elements
          to one animation to avoid layout problems
        - For animations with many elements, consider layout carefully.
          For instance, arrange elements horizontally given the canvas's wider width
        - With four or more horizontal elements, put summary text or similar content at the canvas bottom,
          this will effectively reduce the cutting off and overlap problems
    * Consider the synchronization between animations and content.
      When read at a normal speaking pace, the content should align with the animation's progression.
    * Specify the language of the {animation_engine} texts, it should be the same with the script
      and the storyboard content(Chinese/English for example)
    * Do not use any matchstick-style or pixel-style animations.
      Use charts, images, industrial/academic-style animations
    * Text box needs to have a background color, and the background must be opaque,
      with high contrast between the text color and the background.
    * The text box should large enough to contain the text
    * Do not create multi-track complex {animation_engine} animations.
      One object per segment, or two to three(NO MORE THAN three!) object arranged in a simple manner,
      {animation_engine} layout rules:
        1. One object in the middle
        2. Two objects, left-right structure, same y axis, same size, for example, text left, chart right
        3. Three objects, left-middle-right structure, same y axis, same size.
           No more than 3 elements in one segment
        ...
        6. Use black fonts, **no gray fonts**
          7. CRITICAL: **NEVER put an element to a corner,
              do use horizonal/vertical grid**
        8. No pie charts should be used, the LLM costs many bugs

- You will be given a script. Your storyboard design needs to be based on the script.
  You can also add some additional information you think is useful

- Review the requirements and any provided documents.
  Integrate their content, formulas, charts, and visuals into the script
  to refine the video's screenplay and animations.
    [CRITICAL]: The {animation_engine} and image generation steps will not receive the original requirements and files.
    Supply very detail information for them, especially any data/points/formulas
    to prevent any mismatch with the original query and/or documentation
...
[
    {{
        ...
        "{animation_engine}": "The animation should ... draw component ...",
    }},
    ...
]
```
...
Now begin:"""
    ...
    pure_color_system = """You are an animation storyboard designer.
Now there is a short video scene that needs storyboard design.
The storyboard needs to meet the following conditions:

- Each storyboard panel will carry a piece of narration, one {animation_engine} animation(optional),
  and one subtitle, even one video(optional)
        * Use clear, high-contrast font colors to prevent text from blending with the background
...
- Each of your storyboard panels should take about 5~8 seconds to read at normal speaking speed.
  Avoid the feeling of frequent switching and static
    * Pay attention to the color and content coordination
      between the background image and the {animation_engine} animation.
    * Based on the background image color, select {animation_engine} color scheme
      to make the foreground as clear as possible.
...
- Review the requirements and any provided documents.
    Integrate their content, formulas, charts, and visuals into the script
    to refine the video's screenplay and animations.
    [CRITICAL]: The {animation_engine} steps will not receive the original requirements and files.
    Supply very detail information for them, especially any data/points/formulas
    to prevent any mismatch with the original query and/or documentation
...
Now begin:"""
    ...
    * The content for video shots should not include cinematic language elements like "camera" or "lens," but should instead be used to narrate the visual story, advance the plot, and deepen the theme""" # noqa
    ...
    async def run(self, messages, **kwargs):
        ...
        for i, segment in enumerate(segments):
            ...
            logger.info(
                ...
                f'Animation requirement: {segment.get(self.engine, segment.get("manim", "No animation"))}'
            )
    ...
    async def add_images(self, segments, topic, script, **kwargs):
        ...
        system = """You are an animated short video storyboard assistant designer.
    Your responsibility is to assist storyboard designers in adding foreground images to storyboards.
    You will be given a storyboard design draft and a list of images from user input that you can freely select and use.

1. {{animation_engine_cap}} animation may contain one or more images; these images come from user's documentation,
   or a powerful text-to-image model
    * If the user's documentation contains any images, the information will be given to you:
        a. The image information will include content description, size(width*height) and filename
          b. Carefully select useful images in each segment as best as you can
              and reference the filename in the `user_image` field

        * User-provided images may be insufficient. Trust text-to-image models to generate additional images
            for more visually compelling videos
    ...
    * Important: Use smaller image sizes for generated images and larger image sizes for user doc images.
            DO NOT crop image to circular**

2. The {{animation_engine}} field is a guidance for subsequent {{animation_engine}} animation generation
        * Ignore the input {{animation_engine}}, completely rewrite the {{animation_engine}} field to ensure that
            images and other {{animation_engine}} animations are neatly arranged. Images should be a reasonable part
            of the overall {{animation_engine}} layout,
            appearing in reasonable positions.
    ...
    * Images must be decorated with frames
        ...
        {{animation_engine_cap}} layouts:
        * Do not create multi-track complex {{animation_engine}} animations.
            One object per segment, or two to three(NO MORE THAN three!) object arranged in a simple manner,
            {{animation_engine}} layout rules:
        1. One object in the middle
          2. Two objects, left-right structure, same y axis, same size,
              for example, text left, chart right
          3. Three objects, left-middle-right structure, same y axis, same size.
              No more than 3 elements in one segment
        4. Split complex animation into several segments
        5. Less text boxes in the animation, only titles/definitions/formulas
...
3. The number of images used for each storyboard doesn't need to be the same,
   and images may not be used at all.

4. To reduce attention dispersion, you only need to focus on the image information and {{animation_engine}} fields,
   and generate three fields: {{animation_engine}}, user_image, and foreground.
   Your return value doesn't need to include content and background.
...
6. Your return length should be the same as the source storyboard length, do not miss any segment.
    If images are not needed, return empty user_image and foreground lists.
...
[
    {{{{
        "index": 1, # index of the segment, start from 1
        "{{animation_engine}}": "The animation should ..., use images to... ",
        "user_image": [
            "user_image1.jpg",
            "user_image2.jpg"
        ]
...
An example of image structures given to the {{animation_engine}} LLM:
```json
[
    {{{{
